...
o tamanho real da região analisada e não a versão redimensionada utilizada no cálculo do $PCC$.


\begin{figure}[H]
\includegraphics[width=\columnwidth]{images/figure3.eps}
\caption{Quando a comparação é maior que $0,8$, incluindo número maiores que $0,925$, significa que o objeto foi encontrado,
se o PCC é menor que $0,8$ a imagem é tida como perdida e se o coeficiente for maior que $0,8$ e menor que $0,925$, então o ROI é atualizado pela imagem de 
maior correlação.}
\label{fig:newroicri}
\end{figure}
...
\begin{algorithm}
 \KwData{$ROI$ inicial, posição $P_0$, região de analise do frame atual ($AR$), posição $P$ e a correlação $r$ entre eles. }
