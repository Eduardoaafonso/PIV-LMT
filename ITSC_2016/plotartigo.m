...
Dfit=polyval(p,E);
    figure(1)
	graph = plot(E, D,'-o', E, Dfit,'-.', E, R,'-s');
	grid;
	hx=xlabel('Analyzed frame');
	hy=ylabel('Departure factor');
	ylim([0 1.2]);
    legend('Departure factor','Polynomial fitting','Normalized real route')
  ...
  figure(2);
  graph = plot (E, V,'-o', E, Vfit,'-.', E, Vnorm,'-s');
  grid;
  hx=xlabel('Analyzed frame');
	hy=ylabel('Velocity of Departure factor');
	ylim([-0.06 0.01]);
    legend('Vel. of departure factor','Vel. of polynomial fitting','Vel. of Normalized real route')
