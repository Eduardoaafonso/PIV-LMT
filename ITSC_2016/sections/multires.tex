...
In the presented algorithm, the match criterion to track target is based on the $PIV$ technique, 
where from two images a ROI and a Window Of Search ($WOS$) are defined. A target (object) is identified 
from a highest value of correlation ($PCC$) between a selected $ROI$ and a $WOS$, which determines a object location.
